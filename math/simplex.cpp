...
public:
    ...
    bool solve() { // 返回目标函数值，如果是nan则无解，是inf则解无穷大
        ...
        for (int cnt = 64; cnt--;) { //卡时用，时限充裕的话可以改大
            ...
            for (int j = 0; j < n; ++j) {
                if (cmp(A[m][j], 0) < 0) {
                    ...
                    for (int i = 0; i < m; ++i) {
                        if (cmp(A[i][j], 0) > 0) {
                            if (k == -1 || cmp(A[k][n] / A[k][j], A[i][n] / A[i][j]) > 0) {
                                k = i;
                            }
                        }
                    }
...
private:
    inline double cmp(double a, double b) const {
